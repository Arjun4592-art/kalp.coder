        ...
        <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8">
            ...
            {pricing.map((p, i) => (
              <Reveal key={p.tier} delay={i * 90}>
                <div className="flex h-full flex-col rounded-2xl border border-[var(--border)] bg-[var(--card)] p-8 text-center">
                  <h3 className="font-display text-base font-semibold">{p.tier}</h3>
                  <p className="mt-4 font-display text-2xl font-semibold text-[var(--accent)]">
                    {p.price}
                  </p>
                  <p className="mt-3 flex-1 text-sm text-[var(--text-muted)]">{p.note}</p>
                </div>
              </Reveal>
            ))}
          </div>
          <Reveal delay={200}>
            <div className="mt-12 flex justify-center">
              <Button href="/contact">Get an exact quote — free 30-min call</Button>
            </div>
          </Reveal>
        </div>
      </section>
